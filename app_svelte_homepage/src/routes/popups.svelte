...
<template>
    <div class="">
        <div class="app-bar-button app-bar-reverse">
            ...
            <Btn text="all" on:click="{(e) => styles.forEach(style => modalStore.add(createModal(style)))}"/>
            {#each styles as style}
                <Btn text={style}
                     color={style}
                     on:click="{(e) => modalStore.add(createModal(style))}"/>
            {/each}
        </div>
        <div class="app-bar-button app-bar-reverse">
            <p class="w-32">Notification</p>
            <Btn text="all" on:click="{(e) => styles.forEach(style => notificationStore.add(createNotification(style)))}"/>
            {#each styles as style}
                <Btn text={style}
                     color={style}
        ...
        <div class="app-bar-button app-bar-reverse">
            ...
            <Btn text="all" on:click="{(e) => styles.forEach(style => bannerStore.add(createBanner(style)))}"/>
            {#each styles as style}
                <Btn text={style}
                     color={style}
                     on:click="{(e) => bannerStore.add(createBanner(style))}"/>
            {/each}
        </div>

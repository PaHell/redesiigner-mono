...
<template>
	{#if display}
		<div class="app-sidebar-ct app-sidebar-{right ? 'right' : 'left'}">
			<button
				class="app-sidebar-bg animate__animated {visible ? 'animate__fadeIn' : 'animate__fadeOut'}"
				on:click={(e) => (visible = !visible)}
			>
				<Icon
					name="cross"
					css="items-start justify-center animate__animated {visible
						? 'animate__bounceIn'
						: 'animate__bounceOut'}{right ? 'Left' : 'Right'}"
				/>
			</button>
			<div
				class="app-sidebar {colored ? 'app-colored' : ''} animate__animated {visible
					? 'animate__bounceIn'
					: 'animate__bounceOut'}{right ? 'Right' : 'Left'}"
			>
				{#if $$slots.header}
					<header>
						<div class="app-top">

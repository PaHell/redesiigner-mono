...
<template>
    <menu class="app-modal app-modal-{color} animate__animated {visible ? 'animate__bounceIn' : 'animate__bounceOut'}">
        <main>
            <Icon name="{icon}" large circle/>
            <div class="mt-3 sm:mt-0 sm:ml-3 text-center sm:text-left">
                <h3 class="mt-4 sm:mt-0 text-xl sm:text-lg leading-6 font-semibold text-gray-900 dark:text-white">
                    {title}
                </h3>

...
<template>
    <div class="app-list-entry-profile">
        <div class="app-img">
            {#if img}
                <img src="{img}"/>
                <div class="app-img-shadow"></div>
            {:else}
                <div class="app-img-placeholder {getColorFromName()} text-white dark:text-gray-900">{getInitials()}</div>
            {/if}
            {#if status}
                <div class="app-status app-status-{status}"></div>
            {/if}
        </div>
        <div class="flex-1">
...
<style global lang="postcss">
    ...
    .app-status {
        ...
        &.app-status-offline { @apply bg-gray-400; }
        &.app-status-online { @apply bg-green-500; }
        &.app-status-away { @apply bg-orange-400; }
        &.app-status-busy { @apply bg-red-500; }
    }
